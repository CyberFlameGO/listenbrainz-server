...
def run_daily_jams(user, service):
    ...
    try:
        if user["export_to_spotify"]:
            token = service.get_user(user, refresh=True)
            ...
            spotify = None
        playlist = generate_playlist("daily-jams", args=[username, user["jam_date"]], upload=True, token=token,
                                     created_for=username, spotify=spotify)
