...
def validate_duration_field(listen, key):
    ...
    if key in additional_info:
        ...
        try:
            # try converting duration to integer, this allows to accept duration if it is a valid integer
            # stored in a string. while this is behaviour is discouraged and undocumented, we need this
            # to work around LFM API quirks. see commit message for details.
            value = int(duration)
            if value <= 0:
